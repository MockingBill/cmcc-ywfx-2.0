 ...
 */
app.use(multer({
    dest: './public/static/file',
    /*rename:function (fieldname,filename) {
        const newName=uuidv3();
        return newName+filename;
    },*/
    filename: function (req, file, cb) {
        const newName=uuidv3();
        var fileFormat = (file.originalname).split(".");
        cb(null,newName+"." + fileFormat[fileFormat.length - 1]);
    },
...
function fileFilter(req,file,cb){
    if(file.extension==='xlsx'||file.extension==='xls') {
        ...
        return cb(new Error('Something went wrong'),null,false);
    }
}
...
hbs.registerHelper('ride', function(value, options) {
    var hei=Math.ceil(value/3);
    return hei*225;
});

hbs.registerHelper('toDate', function(value, options) {
   var str=value.substr(0,4)+"年"+value.substr(4,2)+"月"+value.substr(6,2)+"日";
    return str;
});
